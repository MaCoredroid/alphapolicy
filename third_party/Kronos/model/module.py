...
class BinarySphericalQuantizer(nn.Module):
    def __init__(self, embed_dim, beta, gamma0, gamma, zeta,
        ...
        self.register_buffer('sign_pos_one', torch.tensor(1.0), persistent=False)
        self.register_buffer('sign_neg_one', torch.tensor(-1.0), persistent=False)
    ...
    def quantize(self, z):
        ...
        pos = self.sign_pos_one.to(dtype=z.dtype, device=z.device)
        neg = self.sign_neg_one.to(dtype=z.dtype, device=z.device)
        zhat = torch.where(z > 0, pos, neg)
